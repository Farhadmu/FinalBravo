...
# Data extracted from scripts/seed_questions.py
number_series = [
...
def generate_sql():
    ...
    print(f"-- RESTORING {test_name}")
    print("BEGIN;")
    
    # Get test_id
    print(f"DO $$")
    print(f"DECLARE v_test_id UUID;")  # FIXED: uuid
    print(f"BEGIN")
    print(f"  SELECT id INTO v_test_id FROM tests WHERE name = '{test_name}';")
    print(f"  IF v_test_id IS NULL THEN")
    print(f"    RAISE NOTICE 'Test {test_name} not found';")
    print(f"  ELSE")
    print(f"    DELETE FROM questions WHERE test_id = v_test_id;")
    ...
    for i, q in enumerate(questions_data):
        ...
        order = i + 1
        bank_order = 100 + order  # Arbitrary but consistent bank order
        
        # FIXED: Added id generation and bank_order
        print(f"    INSERT INTO questions (id, test_id, question_text, question_type, options, correct_answer, difficulty_level, \"order\", explanation, created_at, updated_at, bank_order) ")
        print(f"    VALUES (gen_random_uuid(), v_test_id, '{text}', 'mcq', '{opts}'::jsonb, '{correct}', '{difficulty}', {order}, '{expl}', NOW(), NOW(), {bank_order});")
    
    print(f"    UPDATE tests SET total_questions = {len(questions_data)} WHERE id = v_test_id;")
    print(f"  END IF;")
    print(f"END $$;")
    print("COMMIT;")
